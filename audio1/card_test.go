...
func Test_sortPortsByPriority(t *testing.T) {
	...
	t.Run("sort ports by priority descending", func(t *testing.T) {
		...
		// 验证排序结果：500 > 300 > 100
		assert.Equal(t, "port2", card.Ports[0].Name)
		assert.Equal(t, uint32(500), card.Ports[0].Priority)
		assert.Equal(t, "port3", card.Ports[1].Name)
		assert.Equal(t, uint32(300), card.Ports[1].Priority)
		assert.Equal(t, "port1", card.Ports[2].Name)
		assert.Equal(t, uint32(100), card.Ports[2].Priority)
	})
	...
	t.Run("sort mixed input and output ports", func(t *testing.T) {
		...
		// 按 Priority 排序，不区分方向
		assert.Equal(t, "output2", card.Ports[0].Name)
		assert.Equal(t, uint32(400), card.Ports[0].Priority)
		assert.Equal(t, "input1", card.Ports[1].Name)
		assert.Equal(t, uint32(300), card.Ports[1].Priority)
		assert.Equal(t, "output1", card.Ports[2].Name)
		assert.Equal(t, uint32(200), card.Ports[2].Priority)
	})
