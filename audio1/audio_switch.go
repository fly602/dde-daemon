...
func (a *Audio) checkCardIsReady(id uint32) bool {
	...
	logger.Warningf("card %s hasSink: %v, isSinkReady: %v; hasSource: %v, isSourceReady: %v",
		card.Name, hasSink, isSinkReady, hasSource, isSourceReady)

	logger.Warningf("check card %s is finaly ready: %v", card.Name, sinkOk && sourceOk)
